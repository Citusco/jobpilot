...
- [X] T008 Request an independent review of the tests added in T001-T003 from the `test-reviewer` subagent (Constitution V). Findings: coverage adequate for the mockable scope; 4 real gaps found and fixed — (1) empty-string `text` (FR-006/AC-4) was never tested, added; (2) `extractResponseSchema`/`jdSubmissionRequestSchema` had no dedicated boundary-case tests (empty tags, non-positive `suggestedQuestionCount`, >6 directions, missing nested fields) — added `tests/unit/agent-orchestration/extract-response.schema.test.ts` and `tests/unit/jd-submissions/jd-submission-request.schema.test.ts`; (3) the client's success test only covered the `sufficient:false` shape — added a `sufficient:true` case; (4) the contract test's 422 case didn't assert persistence — added. Also asked the reviewer whether the contract test's `Test.createTestingModule`+`overrideProvider` DI path would have caught the `tsx` decorator-metadata bug (T007) — answer: no, because the divergence is between two different compilers (`ts-jest`'s real `tsc` vs. `tsx`'s esbuild), not between fake and real DI resolution; closing that residual risk fully would need an e2e smoke test through the same entrypoint/transform `npm run dev` uses, which is a follow-up beyond this task's scope, not something to add silently here.
- [X] T009 [P] If implementation surfaced a new technical pattern or pitfall (e.g. contract-first development against a service that doesn't exist yet, `AbortController` timeout patterns), offer to generate a learning note before closing out (CLAUDE.md Workflow). User was asleep (auto mode) — generated two Obsidian notes rather than blocking on asking, flagged for review: `jobpilot-contract-first-service-boundary.md` (contract-first dev + AbortController/fake-timer testing) and a major update to `jobpilot-nestjs-esm-tooling-pitfalls.md` (the tsx/esbuild decorator-metadata bug from T007).
