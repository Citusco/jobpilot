...
- [ ] T007 [P] Manually run quickstart.md's runnable scenarios (§2 invalid request, §3 upstream failure with `AGENT_SERVICE_URL` pointing nowhere, §4 automated tests) against a running `npm run dev` server; confirm no `jd_submissions` row is written on §3 via `psql`
