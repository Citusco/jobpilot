...
- Before marking any ticket as Done, the ticket-wrapup skill must run: explain what was implemented and why in plain language, decide whether a learning note is warranted, and only then ask whether to update the Jira ticket status. This step is mandatory, not optional, for every ticket.
